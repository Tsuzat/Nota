...
<div
  ...
  {...restProps}
>
  {#each frequencies as freq, idx (idx)}
    <button
    ...
    >
      <span
        class:text-foreground={freq !== frequency}
        class="relative z-10 text-background">{freq}</span
      >

      {#if frequency === freq}
        <layout.span
          class="absolute inset-0 z-1 rounded-lg bg-primary"
