...
import { type ActiveUser, NoteTopbarActions } from '@nota/ui/custom/index.js';
...
import { PUBLIC_COLLABORATION_URL, PUBLIC_NOTA_FRONTEND_URL } from '$env/static/public';
...
function syncUsers() {
  ...
  if (
    ...
    list.every(
      (u, i) =>
        u.clientId === activeUsers[i]?.clientId && u.name === activeUsers[i]?.name && u.color === activeUsers[i]?.color
    )
  ) {
    return;
  }
...
$effect(() => {
  if (note?.id && isPro) {
    versionsClient.getVersionCount(note.id).then((c) => {
      versionCount = c;
    });
  } else {
    versionCount = 0;
...
const getAssets = async (fileType: FileType): Promise<string[]> => assetsByType.get(fileType) ?? [];
...
async function setupEditor(nextId: string): Promise<void> {
  ...
  if (currentNoteId === nextId && currentProviderName === providerName && editor) {
    return;
  }
...
const noteWorkspace = $derived(cloudWorkspaces.workspaces.find((w) => w.id === note?.workspace_id));
    ...
    <Topbar showSeparator={true}>
      ...
      {#snippet right()}
        <NoteTopbarActions
          ...
          canShare={note?.owner === data.user.id ||
            collaborators.members.find((m) => m.user_id === data.user.id)
              ?.role === "admin"}
