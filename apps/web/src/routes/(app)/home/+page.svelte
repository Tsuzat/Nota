...
import { openGlobalSearch } from '$lib/components/sidebar/global-search';
...
// Recent notes: sorted by updated_at (top 12)
const recentNotes = $derived.by(() => {
  const allNotes = [...cloudNotes].filter((n) => !('deleted_at' in n && n.deleted_at));
  return allNotes
...
const pinnedNotes = $derived.by(() => {
  const allNotes = [...cloudNotes].filter((n) => n.pinned && !('deleted_at' in n && n.deleted_at));
...
const totalNotes = $derived([...cloudNotes].filter((n) => !('deleted_at' in n && n.deleted_at)).length);
  ...
  <div class="mx-auto flex w-full max-w-3xl flex-col gap-6 px-6 py-4">
    ...
    <section class="flex flex-col gap-3">
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {#if allCloudWorkspaces.length === 0}
          <div
