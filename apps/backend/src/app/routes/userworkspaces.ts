...
app.get('/', async (c) => {
  ...
  try {
    const userWorkspaces = await DB.select()
      .from(userworkspaces)
      .where(eq(userworkspaces.owner, userId));
...
app.post('/', zValidator('json', schema), async (c) => {
  ...
  try {
    ...
    console.error('Error creating userworkspace:', error);
...
app.delete('/:id', async (c) => {
  ...
  try {
    ...
    console.error('Error deleting userworkspace:', error);
...
app.patch('/:id', zValidator('json', schema.partial()), async (c) => {
  ...
  try {
    ...
    console.error('Error updating userworkspace:', error);
