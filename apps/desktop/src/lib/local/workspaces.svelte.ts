...
class WorkSpaces {
  ...
  async createWorkspace(icon: string, name: string) {
    try {
      ...
      const res = await DB.execute('INSERT INTO workspaces (id, name, icon) VALUES ($1, $2, $3)', [
        id,
        name,
        icon,
      ]);
      if (res.rowsAffected === 1) {
        const newWorkspace = await DB.select<LocalWorkSpace[]>('SELECT id, name, icon, created_at, updated_at FROM workspaces WHERE id = $1', [id]);
