import { convertHtmlToPdf } from '@nota/client';
import type { Content, Editor } from '@nota/ui/edra/index.js';
...
export async function writeBinaryToFile(data: Uint8Array, name: string) {
  const path = await open({
    directory: true,
    multiple: false,
    canCreateDirectories: true,
  ...
  if (path === null) return;
  const resolvedPath = await resolve(path, name);
  toast.promise(writeFile(resolvedPath, data), {
    loading: 'Exporting data...',
    success: 'Exported Successfully',
    error: (err) => {
      console.error(err);
...
export async function exportContent(editor: Editor, name: string, type: 'JSON' | 'HTML' | 'TEXT' | 'MD' | 'PDF') {
  let content: string;
  switch (type) {
    case 'PDF': {
      const toastId = toast.loading('Generating PDF...', { duration: 10000 });
      try {
        const pdfBuffer = await convertHtmlToPdf(name, editor.getHTML());
        toast.success('Exporting PDF...', { id: toastId });
        await writeBinaryToFile(new Uint8Array(pdfBuffer), `${name}.pdf`);
      } catch (err) {
        console.error(err);
