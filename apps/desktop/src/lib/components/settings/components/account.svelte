...
<div class="w-full max-w-xl mx-auto space-y-5 pb-8">
	...
	<div class="space-y-0.5">
		...
		<span class="text-xs text-muted-foreground leading-snug">Manage your cloud profile, subscription plan, and device synchronization.</span>
	</div>

	{#if user}
		<div class="rounded-xl border border-border/40 bg-muted/10 p-5 space-y-5">
			<div class="flex items-center gap-4">
				<Avatar.Root class="size-16 border-2 border-primary/20 ring-4 ring-primary/5 shadow-sm">
					<Avatar.Image src={user.avatar_url} alt={user.name ?? 'User Avatar'} />
				...
				<div class="space-y-0.5">
					...
					{#if user.ai_credits !== undefined}
						<div class="flex items-center gap-1.5 text-xs text-muted-foreground pt-0.5">
							...
							<span>{user.ai_credits.toLocaleString()} AI Credits Available</span>
						</div>
					{/if}
				</div>
			</div>

			<div class="pt-4 border-t border-border/40 flex flex-wrap items-center justify-between gap-3">
				<div class="flex items-center gap-2">
	...
	{:else}
		<div class="flex flex-col items-center text-center space-y-5 py-4 px-4">
			...
			<div class="flex flex-col items-center space-y-2.5">
				<div class="flex size-14 items-center justify-center rounded-2xl bg-primary/10 text-primary border border-primary/20 shadow-xs">
					<icons.CloudSync class="size-7" />
				</div>
				<div class="space-y-0.5 max-w-sm">
					<h3 class="font-semibold text-lg tracking-tight text-foreground">Sign in to Nota</h3>
					<p class="text-xs text-muted-foreground leading-snug">
						Sync your notes seamlessly across desktop and mobile with end-to-end cloud backup.
					</p>
				</div>
			</div>
			...
			<!-- Privacy Footer -->
			<div class="flex items-center justify-center gap-1.5 text-[11px] text-muted-foreground/70 pt-1">
