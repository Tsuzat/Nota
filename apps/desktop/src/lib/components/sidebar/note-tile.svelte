<script lang="ts">
  ...
  import { getNotesContext, getAuthContext, type Note } from "@nota/client";
  ...
  import { openNewNote, openMoveNote, openRenameNote } from "../dialogs";
  ...
  async function deleteNote() {
    try {
      ...
      if (!confirm) return;
      if (page.url.pathname.endsWith(`note-${note.id}`))
        goto(resolve("/"));
      if (isCloud) {
