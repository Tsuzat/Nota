...
const href = $derived(
  isCloud ? resolve('/(cloud)/note-[id]', { id: note.id }) : resolve('/(local)/local-note-[id]', { id: note.id })
);
const isActive = $derived(page.url.pathname.endsWith(href));

const openInBrowser = async () => {
  const url = `${PUBLIC_NOTA_FRONTEND_URL}/n/${note.id}`;
  await invoke('plugin:shell|open', { path: url });
};

const childNotes = $derived.by(() => {
  const allNotes = isCloud ? cloudNotes.notes : localNotes.getNotes();
  return allNotes.filter((n) => n.parent_note_id === note.id && !n.deleted_at);
});
...
async function trashNote() {
  try {
    const confirm = await ask('You will still be able to access the note from the trash. Do you want to continue?', {
...
async function deleteNote() {
  try {
    const confirm = await ask('This note will be deleted permanently. Do you want to continue?', {
