...
async function loadLocalNotes(): Promise<LocalNote[] | null> {
  try {
    let res = await DB.select<LocalNote[]>('SELECT id, workspace_id, parent_note_id, name, icon, pinned, deleted_at, created_at, updated_at FROM notes');
    res = res.map((r) => {
      return {
        ...r,
...
export const load = async () => {
  ...
  const localWorkspaces = await loadLocalWorkspaces();
