<script lang="ts">
  ...
  import StorageViewer, {
    type StorageAssetItem,
  } from "@nota/ui/custom/StorageViewer.svelte";
  ...
  function handleDeleteClick(id: string) {
    if (selectedType === "local") {
      ...
      cloudStorage
        ...
        .then(() => {
          if (authContext.user) {
            authContext.user.used_storage = Math.max(
              0,
              authContext.user.used_storage - size,
            );
          }
        })
        .catch((err) => {
  ...
  const currentAssets = $derived(
    selectedType === "local" ? localAssets : cloudStorage.assets,
  );
  const currentTotal = $derived(
    selectedType === "local" ? localAssets.length : cloudStorage.total,
  );
  const currentUsedStorage = $derived(
    selectedType === "local" ? localUsedBytes : (cloudStorage.usedBytes ?? 0),
  );
  const currentAssignedStorage = $derived(
    selectedType === "local"
      ? localUsedBytes
      : (authContext.user?.assigned_storage ?? 0),
  );
</script>
