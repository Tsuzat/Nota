...
const contentToRestoreUpdate = async (content: Content): Promise<string> => {
  ...
  const ydoc = TiptapTransformer.toYdoc(content, 'default', previewEditor?.extensionManager.extensions);
...
async function performDelete() {
  ...
  try {
    await versionsProvider.deleteVersion(versionToDelete.note_id, versionToDelete.id);
...
async function restoreVersion(v: NoteVersion) {
  ...
  restoringId = v.id;
  try {
    console.log(`[restore] Initiating restore for noteId=${v.note_id}, versionId=${v.id}`);
    ...
    if (viewSource === 'local' && !isLocal) {
      ...
      await versionsCtx.restoreFromContent(v.note_id, content, 'Restored from local snapshot', restoreUpdate);
            ...
            <Card>
              ...
              >
                ...
                <div class="flex items-center gap-2 w-full sm:w-auto">
                  ...
                  {/if}
                  {#if isLocal || (noteMap.get(v.note_id) as unknown as { owner?: string })?.owner === authContext.user?.id}
                    <Button
                      variant="destructive"
                      size="sm"
