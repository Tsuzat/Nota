...
<div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="min-h-0 max-h-[calc(100vh-4rem)] flex-1 overflow-y-auto">
        <div
            class="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-8 sm:px-6"
        >
            <!-- Greeting & Header -->
            ...
            {#snippet workspaceDeleteButton(workspace: Workspace)}
                ...
                {#if reason}
                    <Tooltip.Root>
                        ...
                        <Tooltip.Content
                            class="max-w-48 text-balance text-center"
                        >
                            {reason}
                        </Tooltip.Content>
                    </Tooltip.Root>
            ...
            <section class="flex flex-col gap-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <Badge
                            variant="outline"
                            class="rounded-full size-6 text-xs"
                        >
                            {workspaces.length}
                        </Badge>
                    </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {#each Array(3) as idx (idx)}
                            ...
                            >
                                <div class="flex items-center gap-2">
                                    <div
                                        class="size-6 rounded-md bg-muted/60"
                                    ></div>
                                    <div
                                        class="h-4 w-28 rounded bg-muted/60"
                                    ></div>
                                </div>
                                <div
                                    class="h-3 w-16 rounded bg-muted/40 self-end"
                                ></div>
                            </div>
                        {/each}
                    </div>
                    ...
                    <Card.Root class="mx-auto w-full max-w-md border-dashed">
                        <Card.Header class="text-center">
                            ...
                            <Card.Title class="text-base font-semibold"
                                >No workspaces yet</Card.Title
                            >
                            <Card.Description>
                                Create your first workspace to start taking
                                notes.
                            </Card.Description>
                        </Card.Header>
                ...
                {:else}
                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {#each workspaces as workspace (workspace.id)}
                            {@const isCloud = "ownerId" in workspace}
                            {@const isCurrent =
                                workspace.id === workspaceCtx.current?.id}
                            <Card.Root
                                role="button"
                                tabindex={0}
                                class={cn(
                                    ...
                                    isCurrent
                                        ? "border-primary/60 ring-1 ring-muted-foreground"
                                        : "",
                                )}
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        {#each recentNotes as note (note.id)}
                            <div class="group/card relative">
                                ...
                                >
                                    <Card.Root
                                        class="border-border/60 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"
                                    >
                                        <Card.Header class="pb-1 pr-16">
                                            ...
                                            >
                                                <IconsRenderer
                                                    icon={note.icon ??
                                                        "lucide:FileText"}
                                                    class="size-4 shrink-0 text-muted-foreground transition-colors group-hover/card:text-foreground"
                                                />
                                                <span
                                ...
                                <div
                                    class="absolute top-2.5 right-2.5 z-10 flex items-center gap-0.5"
                                >
                                    <Button
                                        variant="ghost"
                                        size="icon-xs"
