 ...
 */
export async function createWorkspace(input: z.infer<typeof insertWorkspaceSchema>) {
  try {
    const workspaceInsert = insertWorkspaceSchema.parse(input);
    const result = await db.insert(workspace).values(workspaceInsert).returning();
...
export async function deleteWorkspace(id: string): Promise<boolean> {
  try {
    const result = await db.delete(workspace).where(eq(workspace.id, id)).returning();
