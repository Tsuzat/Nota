						...
						if (cells) {
							cells.forEach(({ pos }: { pos: number }, index: number) => {
								decorations.push(
									Decoration.widget(pos + 1, () => {
										...
										grip.setAttribute(
											"aria-label",
											strings.extension.table.selectColumn,
										);
							...
							decorations.push(
								Decoration.widget(lastHeaderCell.pos + 1, () => {
									...
									btn.setAttribute(
										"aria-label",
										strings.extension.table.addColumn,
									);
									btn.setAttribute(
										"title",
										strings.extension.table.addColumnAfter,
									);
									btn.textContent = "+";
				...
				props: {
					handleDOMEvents: {
						mousemove: (view, event) => {
							...
							const grips =
								table.querySelectorAll<HTMLAnchorElement>("a.grip-column");
							grips.forEach((g, idx) => {
							...
							if (wrapper) {
								...
								if (colIndex === lastIndex)
									wrapper.classList.add("last-column-hover");
						...
						mouseleave: (view, event) => {
							...
							const grips =
								table.querySelectorAll<HTMLAnchorElement>("a.grip-column");
							grips.forEach((g) => {
								g.classList.remove("show-col-grip");
							});
							const wrapper = table.closest(".tableWrapper");
