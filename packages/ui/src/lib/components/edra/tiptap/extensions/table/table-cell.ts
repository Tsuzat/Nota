								...
								({ pos }: { pos: number }, index: number) => {
									decorations.push(
										Decoration.widget(pos + 1, () => {
											...
											grip.setAttribute(
												"aria-label",
												strings.extension.table.selectRow,
											);
						...
						if (firstColCells && firstColCells.length > 0) {
							...
							decorations.push(
								Decoration.widget(lastRowCell.pos + 1, () => {
									...
									btn.setAttribute(
										"aria-label",
										strings.extension.table.addRow,
									);
									btn.setAttribute(
										"title",
										strings.extension.table.addRowAfter,
									);
									btn.textContent = "+";
					...
					handleDOMEvents: {
						mousemove: (view, event) => {
							...
							const rowIndex = (cell.parentElement as HTMLTableRowElement)
								.rowIndex;
							const grips =
								table.querySelectorAll<HTMLAnchorElement>("a.grip-row");
							grips.forEach((g, idx) => {
							...
							if (wrapper) {
								const lastIndex = table.rows.length
									? table.rows.length - 1
									: -1;
								if (rowIndex === lastIndex)
									wrapper.classList.add("last-row-hover");
						...
						mouseleave: (view, event) => {
							...
							const grips =
								table.querySelectorAll<HTMLAnchorElement>("a.grip-row");
							grips.forEach((g) => {
								g.classList.remove("show-row-grip");
							});
							const wrapper = table.closest(".tableWrapper");
						...
						mouseout: (view, event) => {
							const target = event.target as HTMLElement;
							const table = target.closest("table");
							const to = (event as MouseEvent)
								.relatedTarget as HTMLElement | null;
							if (!table) return false;
							if (
								!to ||
								!to.closest("table") ||
								to.closest("table") !== table
							) {
								const grips =
									table.querySelectorAll<HTMLAnchorElement>("a.grip-row");
								grips.forEach((g) => {
