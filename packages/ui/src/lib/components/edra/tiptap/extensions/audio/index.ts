...
export const Audio = Node.create<AudioOptions>({
  ...
  renderHTML({ HTMLAttributes }) {
    return ['audio', { controls: 'true', style: 'width: 100%;', ...HTMLAttributes }, ['source', HTMLAttributes]];
  },
  addCommands() {
    return {
      setAudio:
        ({ src }: { src: string }) =>
        ({ commands }) =>
          commands.insertContent(`<audio preload="none" controls autoplay="false" style="width: 100%;" src="${src}"/>`),
