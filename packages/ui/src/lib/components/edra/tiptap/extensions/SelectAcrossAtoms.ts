...
export function atomPenetrationDepth(clientX: number, clientY: number, rect: DOMRectReadOnly): number {
...
export function entrySideFromSelection(sel: Selection, nodeStart: number, nodeEnd: number): AtomVerticalSide | null {
...
export function includeAtomInDragSelection(view: EditorView, nodeStart: number, nodeEnd: number): boolean {
  const { state } = view;
...
function atomHitFromPos(view: EditorView, pos: number): { nodeStart: number; nodeEnd: number; dom: Element } | null {
...
function restorePreservedSelection(view: EditorView, preserved: PreservedRange): void {
  ...
  try {
    ...
    try {
      const next = TextSelection.between(state.doc.resolve(preserved.anchor), state.doc.resolve(preserved.head));
      view.dispatch(state.tr.setSelection(next).setMeta('addToHistory', false));
    } catch {
      /* ignore */
    }
...
function selectAtomOnClick(view: EditorView, event: MouseEvent): boolean {
  ...
  try {
    view.dispatch(
      view.state.tr.setSelection(NodeSelection.create(view.state.doc, hit.nodeStart)).setMeta('pointer', true)
    );
    return true;
  } catch {
    return false;
