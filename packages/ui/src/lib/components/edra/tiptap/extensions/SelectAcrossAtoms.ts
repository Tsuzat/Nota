...
export function leaveSideFromPoint(
	clientY: number,
	rect: DOMRectReadOnly,
): AtomVerticalSide {
...
export function includeAtomInDragSelection(
	...
	try {
		const next = selectionCoveringNode(
			state.doc,
			sel.anchor,
			nodeStart,
			nodeEnd,
...
function shouldPreserveSelectionOnRightClick(
	view: EditorView,
	event: MouseEvent,
): boolean {
...
function restorePreservedSelection(
	...
	try {
		let next: TextSelection = TextSelection.create(
			state.doc,
			preserved.anchor,
			preserved.head,
		);
...
function selectAtomOnClick(view: EditorView, event: MouseEvent): boolean {
	...
	if (
		!hit ||
		!pointInRect(event.clientX, event.clientY, hit.dom.getBoundingClientRect())
	) {
		return false;
	}
	...
	addProseMirrorPlugins() {
		...
		const trackAtomUnderPointer = (
			...
			if (
				!hit &&
				visit &&
				pointInRect(clientX, clientY, visit.dom.getBoundingClientRect())
			) {
				hit = {
			...
			if (hit) {
				...
				if (!visit || visit.nodeStart !== hit.nodeStart) {
					...
					const entrySide = entrySideFromSelection(
						sel,
						hit.nodeStart,
						hit.nodeEnd,
					);
			...
			new Plugin({
					...
					handleDOMEvents: {
						...
						mouseup(view, event) {
							...
							if (dragging && visit) {
								...
								const threshold = effectivePenetrationThreshold(
									visit.dom.getBoundingClientRect(),
								);
						...
						mousemove(view, event) {
							if (!dragging || event.buttons !== 1 || !view.editable)
								return false;
				...
				view(editorView) {
					const onMove = (event: MouseEvent) => {
						if (!dragging || event.buttons !== 1 || !editorView.editable)
							return;
