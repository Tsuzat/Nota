...
export const isRowActiveFromSelection = ({
	...
	const anchorDom = view.domAtPos(state.selection.$from.pos)
		.node as HTMLElement;
...
export const isColumnActiveFromSelection = ({
	...
	const headerCells = Array.from(headerRow.children).filter(
		(el) =>
			(el as HTMLElement).tagName === "TD" ||
			(el as HTMLElement).tagName === "TH",
	);
...
export const moveColumnLeft = (tr: Transaction) => {
	...
	for (let i = 0; i < sourceCells!.length; i++) {
		...
		tr = tr.replaceWith(
			posA,
			posA + nodeA.nodeSize,
			(nodeB as Node).copy((nodeB as Node).content),
		);
