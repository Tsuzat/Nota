<script lang="ts">
  ...
  import { NodeViewWrapper, useEditorTransaction } from "../../tiptap/index.js";
...
<NodeViewWrapper class="my-2 w-full!">
  ...
  <div
    ...
    tabindex={1}
    class={cn(
      "flex min-h-14 w-full items-center gap-2 rounded-lg border border-dashed bg-muted/30 p-4 transition-colors hover:bg-muted/50",
      isUploading && "pointer-events-none opacity-80",
    )}
    onclick={() => {
      if (!isUploading) open = true;
    }}
  >
    {#if isUploading}
      <BarSpinner size={18} class="text-muted-foreground shrink-0" />
      <span
            ...
            <Tabs.Root value="link" class="w-full">
              <Tabs.List>
                ...
                {#if mediaType !== "iframe"}
                  ...
                  {#if getAssetsFn}
                    <Tabs.Trigger value="assets" onclick={loadAssets}
                      >Assets</Tabs.Trigger
                    >
                  {/if}
                {/if}
              ...
              <Tabs.Content value="link">
                <form class="flex flex-col gap-2">
                  <Input type="url" placeholder="Enter URL" bind:value={url} />
                  <Button
                    type="submit"
                    class="capitalize"
                    onclick={() => setMediaFn(url)}>Insert {mediaType}</Button
                  >
                </form>
              </Tabs.Content>
              {#if mediaType !== "iframe"}
                <Tabs.Content value="file">
                  <span class="text-xs"
                    >Upload File {page.url.pathname.includes("local")
                      ? "(Unlimited)"
                      : "(Max 50MB)"}</span
                  >
                  {#if selectFileFn}
                    <Button class="w-full" onclick={handleSelectFile}>
                      Upload File
                    </Button>
                  {/if}
                </Tabs.Content>
                {#if getAssetsFn}
                  <Tabs.Content value="assets">
                    <div
                      class="flex max-h-48 flex-col gap-2 overflow-y-auto p-1"
                    >
                      {#if loadingAssets}
                        <div
                        ...
                        <div class="grid grid-cols-3 gap-2">
                          {#each assets as asset}
                            ...
                            >
                              ...
                              {:else}
                                <span
                                  class="text-[10px] font-mono truncate px-1"
                                  >{asset.split("/").pop()}</span
                                >
                              {/if}
                            </div>
                          {/each}
