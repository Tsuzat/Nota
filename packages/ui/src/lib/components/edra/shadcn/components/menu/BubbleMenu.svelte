<script lang="ts">
  ...
  const shouldShow = (props: {
    ...
    if (propsEditor.isActive("blockMath") || propsEditor.isActive("inlineMath"))
      return false;
...
<BubbleMenu
  ...
  class={cn(
    "flex w-fit max-w-[min(48rem,calc(100vw-2rem))] overflow-x-auto items-center rounded-lg border bg-popover",
    className,
  )}
>
  {#if useAI()}
