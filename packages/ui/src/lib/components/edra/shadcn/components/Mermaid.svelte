<script lang="ts">
  ...
  function downloadImage() {
    ...
    const width =
      viewBoxWidth && viewBoxWidth > 0 ? viewBoxWidth : rect.width || 800;
    const height =
      viewBoxHeight && viewBoxHeight > 0 ? viewBoxHeight : rect.height || 600;
...
>
  {#if isEditing && isEditable()}
    <!-- Editing Mode -->
    <div
      class="flex h-112 w-full flex-col overflow-hidden rounded-lg border bg-background"
    >
      <!-- Toolbar -->
      <div
        class="flex items-center justify-between border-b bg-muted/30 px-3 py-1.5"
      >
      ...
      <div class="flex min-h-0 flex-1 overflow-hidden">
        ...
        {#if mode === "both" || mode === "preview"}
          ...
          >
            {#if error}
              <div
                class="flex max-w-xs flex-col items-center gap-2 text-center"
              >
                <div
                  class="flex size-8 items-center justify-center rounded-lg bg-destructive/10"
                >
    ...
    <div class="group/preview relative w-full">
      ...
      {:else}
        <div class="overflow-hidden rounded-lg border">
          ...
          {#if error}
            <div
              class="flex items-center gap-2 border-t bg-destructive/5 px-4 py-2"
            >
