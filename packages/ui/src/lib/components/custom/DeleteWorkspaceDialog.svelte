...
<script lang="ts">
  ...
  const requiredPhrase = $derived(
    options ? `Delete ${options.workspaceName}` : "",
  );
  ...
  <Dialog.Content showCloseButton={true} class="max-w-md gap-5">
    <Dialog.Header>
      <div class="flex items-center gap-2">
        <div
          class="bg-destructive/10 flex size-9 shrink-0 items-center justify-center rounded-lg"
        >
      ...
      <Dialog.Description class="text-muted-foreground text-sm leading-relaxed">
        This action is <strong class="text-foreground">permanent</strong>. All
        notes and assets in
        <strong class="text-foreground">{options?.workspaceName ?? ""}</strong> will
    ...
    {#if isBlocked}
      <div
        class="flex items-center gap-2.5 rounded-lg border border-amber-300/50 bg-amber-50/80 p-3 dark:border-amber-700/50 dark:bg-amber-950/30"
      >
        <TriangleAlertIcon
    ...
    {:else}
      <div class="flex flex-col gap-2">
        <p class="text-muted-foreground text-xs">
          To confirm, type
          <kbd
            class="bg-muted text-foreground rounded px-1.5 py-0.5 font-mono text-xs font-semibold"
          >
            {requiredPhrase}
          </kbd>
          below:
