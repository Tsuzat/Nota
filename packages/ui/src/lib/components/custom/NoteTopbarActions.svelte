<script lang="ts">
  ...
  const userCount = $derived(
    activeUsers?.length || (connectionStatus === "connected" ? 1 : 0),
  );
...
<div class="flex items-center gap-1">
  ...
  {#if activeUsers}
    <Popover.Root>
      <Popover.Trigger
        class="inline-flex h-8 items-center gap-2 rounded-md border px-2.5 text-xs font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none mr-1 {connectionStatus ===
        'connected'
          ...
          <span class="hidden items-center -space-x-1.5 sm:flex">
            {#each visibleUsers as member (member.clientId ?? member.name)}
              <Avatar class="h-6 w-6 border-2 border-background shadow-sm">
                ...
                <AvatarFallback
                  class="text-[10px]"
                  style="background-color: {member.color}; color: white;"
                >
                ...
                >
                  <Avatar class="h-8 w-8 shadow-sm">
                    ...
                    <AvatarFallback
                      ...
                      style="background-color: {u.color}; color: white;"
                    >
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-1.5">
                      <span class="truncate text-sm leading-none font-medium"
                        >{u.name || "Unknown"}</span
                      >
                    ...
                    <span
                      class="flex items-center gap-1 text-[11px] text-muted-foreground mt-1"
                    >
    ...
    <Popover.Root bind:open={shareOpen}>
      ...
      <Popover.Content align="end" class="w-100 p-0">
        <Tabs.Root value="share" class="w-full">
          ...
          <!-- Share Tab -->
          <Tabs.Content value="share" class="p-4 flex flex-col gap-4">
            <!-- Invite by email -->
            <div class="space-y-2">
            ...
            <div class="space-y-2">
              ...
              {:else}
                <div class="max-h-48 space-y-3 overflow-auto pr-1">
          ...
          <!-- Publish Tab -->
          <Tabs.Content value="publish" class="p-4 space-y-4">
